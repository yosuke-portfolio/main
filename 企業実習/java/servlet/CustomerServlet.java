...
public class CustomerServlet extends BaseServlet {
    ...
    */
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,
        ...
        // 新規登録
        case "new":
            procNew(request, response, session);
            break;
        // 新規登録完了/新規登録未完了
        case "add":
        ...
        // 削除確認
        case "delete_confirm":
            procDeleteConfirm(request, response, state[1], session);
            break;
        // 削除完了/削除未完了
        case "delete":
    ...
    */
    private void procUpdate(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        // TODO 未実装 済 2020/10/03
        CustomerLogic customerLogic = new CustomerLogic();
        CustomerBean customerEdit=(CustomerBean)session.getAttribute("customer");
    ...
    */
    private void procAdd(HttpServletRequest request, HttpServletResponse response, HttpSession session)
            throws ServletException, IOException {
        // TODO 未実装 済 2020/10/3
        CustomerLogic customerLogic = new CustomerLogic();
        CustomerBean customerEdit=(CustomerBean)session.getAttribute("customer");
        String errMessage = null;
        errMessage = customerLogic.add(customerEdit);
        session.removeAttribute("customer");
    ...
    */
    private void procEdit(HttpServletRequest request, HttpServletResponse response, HttpSession session,
            ...
            IOException {
        // TODO 未実装 済 2020/10/5
        int intCustomerId = Integer.parseInt(id);
        CustomerLogic customerLogic = new CustomerLogic();
        CustomerBean customerEdit = null;
        String errMessage = null;
        customerEdit = customerLogic.load(intCustomerId);

        if (customerEdit == null) {
            session.setAttribute("errMessage", errMessage);
            getServletContext().getRequestDispatcher("/WEB-INF/customer/error.jsp").forward(request, response);
        } else {
    ...
    */
    ...
    private void procNewConfirm(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws ServletException,
